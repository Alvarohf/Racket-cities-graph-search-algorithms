...
;-------------------------------- FUNCIONES PARA GENERAR UN GRAFO EN GRAPHVIZ JUNTO AL CAMINO -----------------------------------------

;Funcion que convierte el grafo en uno interpretable por la libreria graph
(define (convertir-grafo grafo)
  (cond
    ;Si es vacio devuelve el grafo
    [(equal? (length grafo) 0) grafo]
    ;Si no va anadiendo las distintas lineas con destinos que se le pasan de forma recursiva
    [else (append (convertir-linea (cdar grafo) (caar grafo))(convertir-grafo (cdr grafo)))]
  )
)
;Funcion auxiliar de convertir grafo que convierte las lineas que componen el grafo mayor
(define (convertir-linea grafo nombre)
  (cond
    ;Si es vacio el grafo se devuelve el grafo
    [(equal? (length grafo) 0) grafo]
    ;Si no se construye con el formato adecuado a partir de la distancia dada y nombres
    [else (cons (list (cadar grafo) nombre (caar grafo)) (convertir-linea (cdr grafo) nombre))]
  )
)
...
(define (elegir-busqueda busqueda cerrado dot)
(cond
  ...
  [(equal? cerrado "no")
   (cond
     ...
     ;Se hara la busqueda por el metodo y con el camino que se devuelva se obtendra el graphViz correspondiente con el camino resaltado
        [(equal? "optimal" busqueda)(mostrar-camino-grafico dot (nodo-camino (busqueda-optimal (list nodo-inicial))))]
        [(equal? "anchura" busqueda)(mostrar-camino-grafico dot (nodo-camino (busqueda-en-anchura (list nodo-inicial))))]
        [(equal? "profundidad" busqueda)(display "CUIDADO pueden producirse bucles infinitos")(mostrar-camino-grafico dot (nodo-camino (busqueda-en-profundidad (list nodo-inicial))))]
        [(equal? "bidireccional" busqueda)(mostrar-camino-grafico dot (nodo-camino (busqueda-bidireccional(list nodo-inicial) (list nodo-final))))]
  ...
  [(equal? cerrado "si") (cond
     ;Comprobamos el tipo de busqueda que se quiere
     ;Se hara la busqueda por el metodo y con el camino que se devuelva se obtendra el graphViz correspondiente con el camino resaltado
      [(equal? "optimal" busqueda)(mostrar-camino-grafico dot (nodo-camino (busqueda-optimal-cerrados (list nodo-inicial) (list) (length grafo))))]
      [(equal? "anchura" busqueda)(mostrar-camino-grafico dot (nodo-camino (busqueda-en-anchura-cerrados (list nodo-inicial) (list) (length grafo))))]
      [(equal? "profundidad" busqueda)(display "CUIDADO pueden producirse bucles infinitos")(mostrar-camino-grafico dot (nodo-camino (busqueda-en-profundidad-cerrados (list nodo-inicial) (list) (length grafo))))]
      [(equal? "bidireccional" busqueda)(mostrar-camino-grafico dot (nodo-camino (busqueda-bidireccional(list nodo-inicial) (list nodo-final))))]
      [else "ERROR: No se puede generar porque la busqueda no es correcta"]
  )]
  [else "ERROR: No se puede generar porque no indico correctamente si queria usar cerrados"])
)
(display  (string-append "\nGrafo en graphViz:\n------------------\n" (elegir-busqueda busqueda cerrado dot)))